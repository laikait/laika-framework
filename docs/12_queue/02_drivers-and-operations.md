...
`pop()` claims the oldest available row with a `SELECT` then an `UPDATE`, inside a transaction but **without a row lock** (`FOR UPDATE`), so that it works on every database the Model module supports. Two workers on the same queue can therefore both claim the same row — run one worker per queue. As with `json`, a job whose worker dies stays reserved.
...
A driver implements `Laika\Engine\Queue\Interfaces\QueueDriverInterface`:
...
- If the driver holds a connection, also implement `Laika\Engine\Queue\Interfaces\ReconnectableDriver` (`reconnect(): void`); the worker calls it in each forked child.
- Throw `Laika\Engine\Queue\Exceptions\DriverException` for misconfiguration.

Register the driver under a name with `Queue::extend()`, from a relay provider's `boot()`, then name it in `lf-config/queue.php`. `php worker`, the `queue:*` commands and `Queue::driver()` in the web app all resolve drivers through `Laika\Engine\Worker\Queue`, so they all pick it up:

```php
// lf-app/Relay/QueueRelay.php, in boot()
use Laika\Engine\Worker\Queue;

Queue::extend('sqs', fn (array $config) => new SqsDriver($config['sqs'] ?? []));
```
...
The resolver receives the whole `lf-config/queue.php` array and must return a `QueueDriverInterface`. A name that matches a built-in driver replaces it.

A failed-job store implements `Laika\Engine\Queue\Interfaces\FailedJobProviderInterface` (`log`, `all`, `find`, `forget`, `flush`). Register it with `Queue::extendFailed('name', $resolver)` and select it with `'failed_driver' => 'name'`.

## The `worker` Executable

`worker` in the project root is generated by `php laika app:sync` (the same run that writes `laika`), which the root `composer.json` calls from `post-autoload-dump` — so `composer install` or `composer dump-autoload` recreates it. It boots the app through `lf-boot/app.php` (hook files included), trusts the `jobs` resource, builds the driver and failed-job store from `lf-config/queue.php`, and runs one queue: `php worker <queue>`.
