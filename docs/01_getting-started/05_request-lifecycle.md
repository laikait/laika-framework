...
What happens between a request arriving at `index.php` and a response leaving it. Knowing this order answers most "why doesn't my X run?" questions — when hooks run, when routes are loaded, which middleware sees what.
...
index.php
 └─ lf-boot/app.php                         BOOT
     ...
     ├─ function files  (laika-core helpers)
     └─ hook files      (lf-hooks/**, then laika-core's)
...
`index.php` requires `lf-boot/app.php`, which runs the same way for web requests, `php laika` and `php worker` — apart from step 3, which is skipped under the CLI:
...
2. **Autoloader.** Requiring `vendor/autoload.php` runs laika-core's `helpers/loader.php`, which:
   - defines `STORAGE_PATH`, `TEMPLATE_PATH`, `TEMPLATE_CACHE_PATH`, `CONFIG_PATH`, `LANG_PATH` (and `DEBUG = true` if you didn't define it);
   - creates the service container and registers `Laika\Relay\CoreProviders` (every `Laika\Service\*` relay);
...
4. **Functions and hooks.** Requires laika-core's helper functions (`config()`, `named()`, `asset()`, ...), then every hook file: your `lf-hooks/**/*.php` first, then laika-core's own hook file.
...
`Url::dispatch()` (`Laika\Route\Dispatcher`) handles the rest:
...
8. **Rendering.** The final string is rendered according to `Laika\Service\Response::getContentType()`:
...
Any uncaught exception or fatal error goes to `Laika\Core\Exceptions\Handler`: logged to `lf-logs/` when `DEBUG` is on, then rendered as a Whoops page (`DEBUG` on) or a generic 500 page (`DEBUG` off). `HttpException` subclasses set their own status. See [Errors & Logging](../18_errors-and-logging/01_basic.md).
